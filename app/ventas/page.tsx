...
function generarHTMLEImprimir(datos: DatosImpresion, tipo: "presupuesto" | "remito" = "presupuesto") {
  ...
  const html = "<!DOCTYPE html><html><head><style>@page{margin:8mm 12mm}body{font-family:Arial;padding:16px;box-sizing:border-box}.logo{height:100px}.header{display:flex;justify-content:space-between;align-items:center}.header-right{text-align:center}.header-right h2{margin:0}.nro-factura{font-size:13px;color:#555;margin-top:4px}.datos{display:flex;justify-content:center;gap:50px;margin-top:14px}.datos>div{flex:1;max-width:250px;font-size:13px}.datos>div+div{border-left:2px solid #eee;padding-left:20px}.contenido{}table{width:100%;margin-top:20px;border-collapse:collapse}th{border:1px solid #ccc;padding:6px;background:#eee;font-size:12px}td{padding:5px;text-align:center;font-size:12px}.totales{margin-top:20px;display:flex;justify-content:flex-end;page-break-inside:avoid;page-break-before:avoid}.box{width:260px;border-top:2px solid #ccc;padding-top:8px}.box p,.box h2{margin:5px 0;font-size:13px}.acciones{display:flex;gap:10px;margin-bottom:14px}@media print{.acciones{display:none}tr{page-break-inside:avoid}.totales{page-break-inside:avoid;page-break-before:avoid}}</style><script>window.addEventListener('afterprint',function(){window.close();});<\/script></head><body><div class='acciones'><button onclick='window.close()' style='background:#f1f5f9;border:1px solid #d1d5db;border-radius:8px;padding:10px 18px;font-size:14px;font-family:Arial;cursor:pointer;color:#374151;font-weight:600'>&#8592; Cerrar</button><button onclick='window.print()' style='background:#0f172a;border:none;border-radius:8px;padding:10px 20px;font-size:14px;font-family:Arial;cursor:pointer;color:white;font-weight:700'>&#128438; Imprimir</button></div><div class='contenido'><div class='header'><img src='" + logoUrl + "' class='logo'/><div class='header-right'><h2>" + titulo + "</h2><div class='nro-factura'>N " + nroFactura + " | Fecha: " + fecha + "</div>" + badgeCC + "</div></div><div class='datos'><div><b>VETIX Distribuidora</b><br/>Almirante Brown 620<br/>Tel: 2604518157<br/>Email: vetix.cf@gmail.com</div><div><b>Cliente:</b><br/>" + clienteSeleccionado.nombre + " " + clienteSeleccionado.apellido + "<br/>CUIT: " + (clienteSeleccionado.cuit || "-") + "<br/>Direccion: " + (clienteSeleccionado.localidad || "-") + "<br/>Tel: " + (clienteSeleccionado.telefono || "-") + "</div></div><table><thead>" + theadCols + "</thead><tbody>" + filas + "</tbody></table></div>" + totalesHTML + "</body></html>"
