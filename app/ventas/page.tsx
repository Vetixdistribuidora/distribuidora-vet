...
function generarHTMLEImprimir(datos: DatosImpresion, tipo: "presupuesto" | "remito" = "presupuesto") {
  ...
  const html = "<!DOCTYPE html><html><head><style>@page{margin:8mm 12mm}*{box-sizing:border-box}body{font-family:Arial;margin:0;padding:12px;display:flex;flex-direction:column;min-height:calc(100vh - 24px)}.acciones{display:flex;gap:10px;margin-bottom:12px}.logo{height:90px;display:block}.empresa-info{font-size:11px;color:#555;margin-top:5px;line-height:1.7}.header{display:flex;justify-content:space-between;align-items:flex-start}.header-right{text-align:center;padding-top:6px}.header-right h2{margin:0;font-size:18px}.nro-factura{font-size:13px;color:#555;margin-top:4px}.cliente-row{margin-top:12px;padding:8px 12px;background:#f8fafc;border:1px solid #e2e8f0;border-radius:5px;font-size:12px;line-height:1.8}.contenido{flex:1}table{width:100%;margin-top:14px;border-collapse:collapse}th{border:1px solid #ccc;padding:6px 8px;background:#eee;font-size:11px}td{padding:5px 6px;text-align:center;font-size:11px;border-bottom:1px solid #f0f0f0}.totales{margin-top:auto;padding-top:16px;display:flex;justify-content:flex-end;break-inside:avoid;page-break-inside:avoid}.box{width:250px;border-top:2px solid #333;padding-top:8px}.box p,.box h2{margin:4px 0;font-size:12px}@media print{.acciones{display:none}body{min-height:0;padding:0}tr{page-break-inside:avoid;break-inside:avoid}.totales{break-inside:avoid;page-break-inside:avoid;break-before:avoid;page-break-before:avoid}}</style></head><body><div class='acciones'><button onclick='window.close()' style='background:#f1f5f9;border:1px solid #d1d5db;border-radius:8px;padding:10px 18px;font-size:14px;font-family:Arial;cursor:pointer;color:#374151;font-weight:600'>&#8592; Cerrar</button><button onclick='window.print()' style='background:#0f172a;border:none;border-radius:8px;padding:10px 20px;font-size:14px;font-family:Arial;cursor:pointer;color:white;font-weight:700'>&#128438; Imprimir</button></div><div class='contenido'><div class='header'><div><img src='" + logoUrl + "' class='logo'/><div class='empresa-info'>Almirante Brown 620<br/>Tel: 2604518157 &nbsp;|&nbsp; Email: vetix.cf@gmail.com</div></div><div class='header-right'><h2>" + titulo + "</h2><div class='nro-factura'>N " + nroFactura + " | Fecha: " + fecha + "</div>" + badgeCC + "</div></div><div class='cliente-row'><b>Cliente:</b> " + clienteSeleccionado.nombre + " " + clienteSeleccionado.apellido + " &nbsp;|&nbsp; <b>CUIT:</b> " + (clienteSeleccionado.cuit || "-") + " &nbsp;|&nbsp; <b>Dir:</b> " + (clienteSeleccionado.localidad || "-") + " &nbsp;|&nbsp; <b>Tel:</b> " + (clienteSeleccionado.telefono || "-") + "</div><table><thead>" + theadCols + "</thead><tbody>" + filas + "</tbody></table></div>" + totalesHTML + "</body></html>"
