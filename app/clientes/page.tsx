  ...
  return (
    <div>
      ...
      <div>
        ...
        <input placeholder="% Descuento" onChange={e => setDescuento(e.target.value)} />
      ...
      <div style={{ marginTop: 20 }}>
        {clientes.map(c => (
          <div key={c.id} style={{ background: "white", padding: 15, marginBottom: 10 }}>
            <b>{c.nombre} {c.apellido}</b>
            <p>CUIT: {c.cuit}</p>
            <p>📞 {c.telefono}</p>
            <p>📍 {c.localidad}</p>
            <p>💸 Descuento: {c.descuento}%</p>
          </div>
        ))}
      </div>
