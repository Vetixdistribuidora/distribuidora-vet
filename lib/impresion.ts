...
export function generarHTMLEImprimir(datos: DatosImpresion, tipo: "presupuesto" | "remito" = "presupuesto") {
  ...
  const saldoHTML = (!esRemito && saldoCliente > 0)
    ? "<div style='margin-top:10px;background:#fff3cd;border:1px solid #e67700;border-radius:6px;padding:8px 12px;text-align:center'><span style='font-size:11px;color:#92400e;font-weight:600'>SALDO TOTAL EN CUENTA CORRIENTE</span><br/><span style='font-size:16px;color:#e67700;font-weight:800'>" + f(saldoCliente) + "</span></div>"
    : ""
  ...
  const totalesHTML = esRemito
    ...
    : "<div class='totales'><div class='box'>" + metodoStr + "<p><b>Subtotal:</b> " + f(subtotal) + "</p><p><b>IVA (" + ivaNum + "%):</b> " + f(subtotal * ivaNum / 100) + "</p><h2><b>Total:</b> " + f(total) + "</h2>" + saldoHTML + "</div></div>"
...
<div class="page">
  ...
  <div class="total-box"><div class="total-inner">
    ...
    ${saldoTotal > 0 ? `<div class="saldo-box saldo-pendiente"><p class="saldo-label">Saldo total en cuenta corriente: ${f(saldoTotal)}</p></div>` : ""}
  </div></div>
  <div class="firma-box">
